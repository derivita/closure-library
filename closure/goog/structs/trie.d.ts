 ...
 * of Trie classes.
 */
export declare class Trie<VALUE = any> {
    /**
     * Class for a Trie datastructure.  Trie data structures are made out of trees
     * of Trie classes.
     * @param opt_trie Optional Trie or Object to initialize trie with.
     */
    constructor(opt_trie?: Trie<VALUE | null> | {
        [key: string]: VALUE | null;
    } | null);
    private noStructuralTyping_closure_goog_structs_trie_Trie;
    /**
    ...
    setAll(trie: {
        [key: string]: VALUE | null;
    } | Trie<VALUE | null>): void;
    /**
     * Retrieves a value from the trie given a key.  O(L), where L is the length of
     * the key.
     * @param key The key to retrieve from the trie.
     * @return The value of the key in the trie, or undefined if the trie does not contain this key.
     */
    get(key: string): VALUE | undefined | null;
    /**
     ...
     * @return Map of end index of matching prefixes and corresponding values. Empty if no match found.  Note that the key is the index of the final character of the entry, rather than the length, so entries with a single character will clobber an empty-string entry with key '0'.
     */
     ...
     * @return Whether the trie contains key.
     */
     ...
     * @return Whether any key of the trie has the prefix.
     */
     ...
     * @return Whether the trie contains the value.
     */
     ...
     * in the trie.
     * @return A new Trie with the same key value pairs.
     */
    clone(): Trie<VALUE | null>;
    /**
     ...
     * @return True iff this trie contains no elements.
     */
    isEmpty(): boolean;
}
