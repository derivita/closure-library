...
export declare class BidiFormatter {
     ...
     *
     * See http://wiki/Main/TemplatesAndBiDi for more information.
     * @param contextDir The context directionality, in one of the following formats: 1. A Dir constant. NEUTRAL is treated the same as null, i.e. unknown, for backward compatibility with legacy calls. 2. A number (positive = LTR, negative = RTL, 0 = unknown). 3. A boolean (true = RTL, false = LTR). 4. A null for unknown directionality.
     ...
     */
    constructor(contextDir: Dir | number | boolean | null | null, opt_alwaysSpan?: boolean);
    private noStructuralTyping_closure_goog_i18n_bidiformatter_BidiFormatter;
    getContextDir(): Dir | null;
    getAlwaysSpan(): boolean;
    /**
     *
     * @param contextDir The context directionality, in one of the following formats: 1. A Dir constant. NEUTRAL is treated the same as null, i.e. unknown. 2. A number (positive = LTR, negative = RTL, 0 = unknown). 3. A boolean (true = RTL, false = LTR). 4. A null for unknown directionality.
     */
    setContextDir(contextDir: Dir | number | boolean | null | null): void;
    /**
     *
     * @param alwaysSpan Whether {@link #spanWrap} should always use a 'span' tag, even when the input directionality is neutral or matches the context, so that the DOM structure of the output does not depend on the combination of directionalities.
     */
     ...
     */
    estimateDirection(a: string, b?: boolean): Dir | null;
    /**
     ...
     * @param opt_isHtml Whether `str` is HTML / HTML-escaped. Default: false.
     * @return "rtl" or "ltr", according to the logic described above.
     */
     ...
     * @param str Text whose directionality is to be estimated.
     * @param opt_isHtml Whether `str` is HTML / HTML-escaped. Default: false.
     * @return 'dir="rtl"' for RTL text in non-RTL context; 'dir="ltr"' for LTR text in non-LTR context; else, the empty string.
     */
     ...
     * @return Input text after applying the processing.
     */
     ...
     * @return Input text after applying the processing.
     */
    spanWrapSafeHtmlWithKnownDir(dir: Dir | null, html: SafeHtml, opt_dirReset?: boolean): SafeHtml;
    /**
     ...
     * @return Input text after applying the above processing.
     */
     ...
     * @return Input text after applying the above processing.
     */
    unicodeWrapWithKnownDir(dir: Dir | null, str: string, opt_isHtml?: boolean, opt_dirReset?: boolean): string;
    /**
     ...
     * @param str The input text.
     * @param opt_isHtml Whether `str` is HTML / HTML-escaped. Default: false.
     * @return A Unicode bidi mark matching the global directionality or the empty string.
     */
     ...
     * @return A Unicode bidi mark matching the global directionality or the empty string.
     */
    markAfterKnownDir(dir: Dir | null, str: string, opt_isHtml?: boolean): string;
    /**
     * Returns the Unicode BiDi mark matching the context directionality (LRM for
     * LTR context directionality, RLM for RTL context directionality), or the
     * empty string for neutral / unknown context directionality.
     * @return LRM for LTR context directionality and RLM for RTL context directionality.
     */
    mark(): string;
    /**
     * Returns 'right' for RTL context directionality. Otherwise (LTR or neutral /
     * unknown context directionality) returns 'left'.
     * @return 'right' for RTL context directionality and 'left' for other context directionality.
     */
    startEdge(): string;
    /**
     * Returns 'left' for RTL context directionality. Otherwise (LTR or neutral /
     * unknown context directionality) returns 'right'.
     * @return 'left' for RTL context directionality and 'right' for other context directionality.
     */
    endEdge(): string;
}
