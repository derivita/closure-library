 ...
 */
export declare class MockClock extends Disposable {
    /**
     * Class for unit testing code that uses setTimeout and clearTimeout.
     *
     * NOTE: If you are using MockClock to test code that makes use of
    ...
    private noStructuralTyping_closure_goog_testing_mockclock_MockClock;
    /**
     ...
     */
    static createAsyncMockClock(): MockClock;
    /**
     ...
     * @param retainAsyncQueue By default, a synchronous MockClock will clear any pending async work when reset. This can leave googPromises in a state where callbacks can never be executed. Set this flag to restore original scheduling behavior and retain the async queue. This argument is ignored for an async-only MockClock.
     */
     ...
     * @param opt_millis Number of milliseconds to increment the counter. If not specified, clock ticks 1 millisecond.
     */
    tickPromise<T = any>(promise: Thenable<T | null>, opt_millis?: number): T | null;
    getTimeoutsMade(): number;
    getCallbacksTriggered(): number;
    getCurrentTime(): number;
    /**
     *
     * @param timeoutKey The timeout key.
     ...
     */
    tickAsyncMustSettlePromise<T = any>(millis: number, promise: Thenable<T | null>): Promise<T | null>;
    /**
     ...
     */
    doTimeWarpAsync(newDate: Date): Promise;
}
export declare namespace MockClock {
    type QueueObjType_ = {
        'timeoutKey': number;
        'millis': number;
        'runAtMillis': number;
